...
## Local development with Docker (DB + Redis)

Run PostgreSQL and Redis in Docker, then start the app on your machine:
...
2. **Configure env** (if not already done)
   ...
   ```
   The default `.env` values match the Docker setup (`localhost:5432`, `localhost:6379`).
...
To stop the containers: `docker compose -f docker-compose.dev.yml down`
